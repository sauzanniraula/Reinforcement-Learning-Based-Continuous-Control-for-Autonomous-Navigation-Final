...
**Prerequisites:**
  ...
  * Your System must have an **GPU** to run CARLA.
  * You must have an **NVIDIA GPU** for the SAC algorithm to train effectively.

**Installation Steps:**

1.  **Create `weights` Folder:**
    Run the setup script one time to create the `weights` folder.
...
2.  **Install Dependencies:**
    Activate your virtual environment (`pme_venv`) and install the required packages.
...
**For Training (Fastest):**
Run CARLA in "headless" mode (no window). This is highly recommended for training.
...
**For Testing/Watching (Slower):**
Run CARLA in a small window.
...
**To Train:**
This is the "school" where your agent learns. This will take many hours.
...
**To Evaluate (The "Driving Test"):**
This loads a saved "brain" from the `weights/` folder and lets you watch it drive.
...
1.  **Find the "Stuck" Process:**
    Run this in your terminal to find all processes using port 2000.

    ```powershell
    netstat -ano | findstr :2000
    ```

    You will see an output like this. The last number is the Process ID (PID).
    `TCP    0.0.0.0:2000           0.0.0.0:0              LISTENING       13600`
    *(In this example, the PID is `13600`)*

2.  **Kill the Process:**
    Use the PID you just found to force-kill the process.
